    ...
    <section id="features" className="bg-neutral-50 py-24">
      <div className="mx-auto max-w-6xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className={"mt-3 text-3xl font-medium tracking-[-0.01em] sm:text-4xl " + brand.heading}>{features.heading}</h2>
        </div>
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-2">
          {features.items.map((f) => (
            <Card key={f.title} className={"border-neutral-200/70 p-8 " + brand.shadow}>
              <h3 className={"text-lg font-semibold " + brand.heading}>{f.title}</h3>
